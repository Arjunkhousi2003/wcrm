...
describe("RATE_LIMITS presets", () => {
  it("send and broadcast batch budgets are independent", async () => {
    __resetRateLimitForTests();
    // Importing here so the presets stay close to their assertions.
    const { RATE_LIMITS } = await import("./rate-limit");
    expect(RATE_LIMITS.send.limit).toBeGreaterThan(
      RATE_LIMITS.broadcastStart.limit,
    );
    expect(RATE_LIMITS.broadcastBatch.limit).toBeGreaterThan(
      RATE_LIMITS.broadcastStart.limit,
    );
    expect(RATE_LIMITS.send.windowMs).toBe(60_000);
    expect(RATE_LIMITS.broadcastStart.windowMs).toBe(60_000);
  });
});
